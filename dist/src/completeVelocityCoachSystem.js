...
import { EnhancedExerciseDetector, getExerciseConfig, getAllExerciseTypes } from './enhancedExerciseSystem';
...
class KinematicAnalyzer {
    ...
    static calculateJointAngle(joint1, joint2, joint3) {
    ...
    static calculateLinearVelocity(angularVelocity, radiusOfRotation) {
        return Math.abs(angularVelocity * radiusOfRotation); // m/s
    }
    // Calculate radius of rotation between joints
    static calculateRadius(shoulder, elbow, wrist) {
    ...
    static classifyMovement(exerciseType) {
...
export class VelocityCoachAI {
    landmarker;
    voiceAgent = new VBTEnhancedVoiceAgent();
    database;
    ...
    createUI() {
        ...
        if (exerciseGrid) {
            ...
            const exercisesByGroup = {
                'Shoulders': ['shoulder_press', 'lateral_raise', 'front_raise', 'rear_delt_fly'],
                'Chest': ['chest_press', 'push_ups'],
                'Arms': ['bicep_curl', 'tricep_extension'],
                'Back': ['rows'],
                'Legs': ['squats']
            };
    ...
    setupEventHandlers() {
        ...
        if (completeSetBtn)
            completeSetBtn.addEventListener('click', () => this.completeCurrentSet());
        if (nextExerciseBtn)
    ...
    analyzeKinematics(landmarks, exercise) {
        const config = getExerciseConfig(exercise);
        const joints = config.landmarks.primary;
        if (joints.length < 3)
            return null;
        // Fixed tuple type assertion
        const joint1 = [landmarks[joints[0]].x, landmarks[joints[0]].y];
        const joint2 = [landmarks[joints[1]].x, landmarks[joints[1]].y];
        const joint3 = [landmarks[joints[2]].x, landmarks[joints[2]].y];
    ...
    highlightActiveJoints(landmarks, w, h) {
        const config = getExerciseConfig(this.currentExercise);
        const joints = config.landmarks.primary;
...
//# sourceMappingURL=completeVelocityCoachSystem.js.map