...
export class VelocityCoachAI {
    ...
    isResting = false;
    isExerciseTransition = false; // NEW: Track exercise transitions
    restStartTime = 0;
    setStartTime = 0; // Track when set actually starts
    restTimer;
    lastRepTime = 0; // Prevent rep spam
    // Exercise tracking
    ...
    processLateralRaise_SERIOUS_FIX(landmarks, timestamp, state) {
        ...
        if (state.angleHistory.length > 7)
        ...
        if (Math.abs(velocity) > 5) {
            ...
            if (state.velocityHistory.length > 15)
    ...
    calculateVelocityLoss(currentVelocity, state) {
        if (!state.velocityHistory || state.velocityHistory.length < 5)
            return 0;
    ...
    startRest(isExerciseTransition) {
        const progress = this.exerciseProgress.get(this.currentExercise);
        this.isResting = true;
    ...
    updateSessionProgress() {
        ...
        if (totalSetsEl)
            totalSetsEl.textContent = this.totalSetsCompleted.toString();
        if (totalRepsEl)
            totalRepsEl.textContent = this.totalRepsCompleted.toString();
...
//# sourceMappingURL=completelyFixedVelocityCoach.js.map