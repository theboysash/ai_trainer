export class VisualInsightSystem {
    ...
    lastUpdate = 0;
    UPDATE_INTERVAL = 1000; // Update insights every second
    update(stepData) {
        const now = Date.now();
    ...
    analyzeVelocityTrends(stepData) {
    ...
    analyzeRangeOfMotion(stepData) {
        ...
        const targetROM = romTargets[stepData.exercise] || 70;
    ...
    analyzeTempo(stepData) {
        const velocityHistory = this.historicalData.get('velocity') || [];
        if (velocityHistory.length < 8)
            return;
    ...
    analyzeFatigueLevel(stepData) {
        const formHistory = this.historicalData.get('formScore') || [];
        const velocityHistory = this.historicalData.get('velocity') || [];
        if (formHistory.length < 10)
            return;
...
//# sourceMappingURL=visualInsights.js.map