// src/curlCounter.ts
export function angleBetween(a: number[], b: number[], c: number[]) {
...
export const euclid2d = (p:[number,number], q:[number,number])=>{
  const dx = p[0]-q[0], dy=p[1]-q[1]; return Math.sqrt(dx*dx+dy*dy);
};
...
export class CurlCounter {
  ...
  state: "idle"|"going"|"peak"|"returning" = "idle";
  reps = 0;
  last_angle?: number;
  last_t?: number;
  velocity = 0;
  ...
  // per-rep tracking
  _start_time?: number|null = null;
  _t_peak?: number|null = null;
  _start_angle?: number|null = null;
  _min_angle?: number|null = null;
  _start_elbow_rel?: [number,number]|null = null;
  _max_elbow_disp = 0;
  _pcv = 0;
  _conc_sum = 0;
  _conc_time = 0;
  ...
  pcv_list: number[] = [];
  mcv_list: number[] = [];
  last_rep_pcv: number|null = null;
  last_rep_mcv: number|null = null;
  ...
  should_flash_banner(nowSec:number){ return nowSec <= this._flash_until; }

  step(angle:number, nowSec:number, elbow_rel?:[number,number]|null, visible=true): StepResult {
    if (this.last_angle == null) {
      this.last_angle = angle; this.last_t = nowSec;
    ...
    if ((this.state==="idle"||this.state==="returning") && down_hit) {
      ...
      if (conc_v > this._pcv) this._pcv = conc_v;
      ...
      if (elbow_rel && this._start_elbow_rel) {
        ...
        if (drift > this._max_elbow_disp) this._max_elbow_disp = drift;
      }
      if (this._min_angle==null || angle < this._min_angle) this._min_angle = angle;
      if (up_hit) { this.state = "peak"; this._t_peak = nowSec; }
      ...
      this._start_time = null; this._t_peak = null;
      this._start_angle = null; this._min_angle = null;
      this._start_elbow_rel = null; this._max_elbow_disp = 0;
      this._pcv = 0; this._conc_sum = 0; this._conc_time = 0;
    }
