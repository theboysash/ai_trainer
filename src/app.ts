...
export class BicepCurlApp {
  // landmark indices
  L_SHO=11; L_ELB=13; L_WRI=15;
  R_SHO=12; R_ELB=14; R_WRI=16;
  ...
  _last_clap_t=0;
  _first_clap_t:number|null=null;
  ...
  private check_double_clap(lm:any[], now:number){
    if (now < this._set_cooldown_until) { this._last_close=false; return false; }
    const lw:[number,number] = [lm[15].x, lm[15].y];
    const rw:[number,number] = [lm[16].x, lm[16].y];
    ...
    if (!close && this._last_close) this._last_far_t = now;
  ...
  private compute_metrics(lm:any[]){
    ...
    const a=[lm[s].x,lm[s].y,lm[s].z]; const b=[lm[e].x,lm[e].y,lm[e].z]; const c=[lm[w].x,lm[w].y,lm[w].z];
    const ang = angleBetween(a,b,c);
    const elbow_rel:[number,number] = [lm[e].x-lm[s].x, lm[e].y-lm[s].y];
