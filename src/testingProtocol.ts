// src/testingProtocol.ts - For development testing
...
export class ExerciseTestRunner {
  ...
  static testExerciseConfigs() {
    ...
    shoulderExercises.forEach(exercise => {
      try {
        const config = getExerciseConfig(exercise as ExerciseType);
  ...
  static testExerciseDetection(exercise: string, testCases: any[]) {
...
if (typeof window !== 'undefined') {
  (window as any).testExerciseSystem = ExerciseTestRunner.testAllExercises;
  (window as any).simulateWorkout = simulateWorkout;
  (window as any).qaChecklist = QA_CHECKLIST;
}