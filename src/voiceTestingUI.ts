// src/voiceTestingUI.ts
import { VoiceAgent, VoiceMessage } from './voiceAgent';
...
export class VoiceTestingUI {
  ...
  private createIntensitySelector(): HTMLElement {
    ...
    const intensities: Array<{name: 'light' | 'moderate' | 'intense', color: string}> = [
  ...
  private createTestButtons(): HTMLElement {
    const section = document.createElement('div');
    section.style.marginBottom = '15px';
    ...
    const tests = [
      { type: 'warning' as const, label: 'VL Warning', icon: '⚠️' },
      { type: 'instruction' as const, label: 'Form Tips', icon: '💪' },
      { type: 'celebration' as const, label: 'Set Complete', icon: '🎉' },
      { type: 'adjustment' as const, label: 'Weight Adj.', icon: '⚖️' },
      { type: 'encouragement' as const, label: 'Motivation', icon: '🔥' }
    ];
  ...
  private createTestButton(label: string, icon: string, action: () => void): HTMLElement {
    const btn = document.createElement('button');
