...
export type ExerciseType = 'bicep_curl' | 'shoulder_press' | 'lateral_raise' | 'front_raise' | 'rear_delt_fly' | 
                          'chest_press' | 'push_ups' | 'tricep_extension' | 'rows' | 'squats';
...
export interface WorkoutSession {
  ...
  userId?: string; // For MongoDB integration
}
...
export class EnhancedExerciseDetector {
  ...
  private computeRearDeltMetrics(landmarks: any[], joints: number[]) {
    return this.computeFrontRaiseMetrics(landmarks, joints); // Similar mechanics
  }

  private computePushUpMetrics(landmarks: any[], joints: number[]) {
  ...
  private computeTricepExtensionMetrics(landmarks: any[], joints: number[]) {
  ...
  private computeRowMetrics(landmarks: any[], joints: number[]) {
  ...
  private computeSquatMetrics(landmarks: any[], joints: number[]) {
...
export function getExercisesByMuscleGroup(muscleGroup: 'shoulders' | 'chest' | 'arms' | 'back' | 'legs'): ExerciseType[] {
  return getAllExerciseTypes().filter(exercise => 
    ENHANCED_EXERCISE_CONFIGS[exercise].muscleGroup === muscleGroup
  );
}