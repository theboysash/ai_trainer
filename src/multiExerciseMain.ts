...
import { getExerciseConfig, getAllExerciseTypes } from "./exerciseConfigs";
...
const btnSkipRest = document.getElementById("btnSkipRest") as HTMLButtonElement;
const btnCompleteSet = document.getElementById("btnCompleteSet") as HTMLButtonElement;
const btnNextExercise = document.getElementById("btnNextExercise") as HTMLButtonElement;
const btnToggleVoice = document.getElementById("btnToggleVoice") as HTMLButtonElement;

// UI Elements
const exerciseGrid = document.getElementById("exerciseGrid") as HTMLElement;
...
function initializeExerciseGrid(): void {
...
function createExerciseCard(config: any): HTMLElement {
  ...
  card.addEventListener('click', () => {
    selectExercise(config.type as ExerciseType);
  });
  
  return card;
}

function selectExercise(exerciseType: ExerciseType): void {
  if (running) {
...
function updateExerciseSelection(): void {
...
function setupWorkoutManagerCallbacks(): void {
...
function drawSkeleton(landmarks: any[], w: number, h: number): void {
  // Draw connections
  ctx.lineWidth = 3;
...
function highlightActiveJoints(landmarks: any[], stepData: any, w: number, h: number): void {
...
function updateRealTimeMetrics(stepData: any): void {
...
function updateCurrentExerciseDisplay(): void {
  const currentExType = workoutManager['session'].currentExercise;
  const config = getExerciseConfig(currentExType);
  const progress = workoutManager.getCurrentExerciseProgress();
...
function updateCurrentSetDisplay(stepData?: any): void {
  if (stepData) {
    if (stepData.isResting) {
...
function showRestTimer(duration: number): void {
...
function updateRestTimer(remaining: number): void {
  if (remaining > 0) {
...
function switchToNextExercise(): void {
