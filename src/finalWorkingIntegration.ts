...
import { ExerciseType, ExerciseDetector } from './exerciseSystem';
...
class VBTIntegratedVoiceAgent {
  ...
  private checkVelocityLoss(stepData: any): void {
    ...
    if (!data || !data.baseline || data.velocityHistory.length < 4) return;
...
class WorkingVisualDashboard {
  ...
  private updateExerciseDisplay(stepData: any): void {
    ...
    if (setStatus) {
      ...
      setStatus.textContent = statusMap[stepData.state as keyof typeof statusMap] || stepData.state;
    }
  }

  private updateStatus(stepData: any): void {
    // Update exercise cards
    document.querySelectorAll('.exercise-card').forEach(card => {
      card.classList.remove('active');
      if ((card as HTMLElement).dataset.exercise === stepData.exercise) {
...
export class VelocityCoachAI {
  ...
  private voiceAgent = new VBTIntegratedVoiceAgent();
  private visualDashboard = new WorkingVisualDashboard();
  ...
  private getVelocityLoss(detector: any): number {
  ...
  private drawOverlayInfo(stepData: any, w: number, h: number): void {
    const ctx = this.ctx;
